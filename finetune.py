...
def main(
    ...
    do_test = False
    set_seed(seed)
    print("Creak PATH: ", creak_dev)
    print("Creak PATH: ", creak_test)
    print("Creak PATH: ", creak_train)
    dataset = load_dataset("csv", data_files={"train": creak_train, "dev": creak_dev, "test": creak_test})
    ...
    def preprocess_function(examples):
        ...
        # inputs = [f"{text_column} : {x} Label : " for x in examples[text_column]] # TODO: remove for creak
    ...
    train_dataloader = DataLoader(
        ...
        batch_size=batch_size, pin_memory=True, sampler=sampler
    )
    ...
    for epoch in range(num_epochs):
        ...
        for eval_mode in ["dev", "test"]:
            eval_model_dataloader = eval_dataloader if eval_mode == "dev" else test_dataloader
            model.eval()
            eval_preds = []
            with TorchTracemalloc() as tracemalloc:
            ...
            for pred, true in zip(eval_preds, dataset[eval_mode][label_column]):
    ...
    model_path = f"/projectnb/llamagrp/feyzanb/peft/trained_models/{dataset_name}_{model_name_or_path}_{peft_config.peft_type}_{peft_config.task_type}"
